...
async def upload(file: UploadFile = File(...)):
    try:
        ...
        for i in files.values:
            ...
            try:
                send_text = requests.post(f'http://localhost:{port}/api/whatsapp/sendtxt', post_json)
                if(send_text.status_code != 201):
                    print(send_text)
                    print(f'Problema Grave con el envio status code: {send_text.status_code}')
                else:
                    print(send_text)
            except Exception as e:
                print(e)
                print('problema no se pudo mandar se va amandar el mensaje de otro puerto')
                ...
                try:
                    ...
                    print(e)
            print(f'Mensaje terminando esperando para el siguiente {random_sync_sleep} segundos')
